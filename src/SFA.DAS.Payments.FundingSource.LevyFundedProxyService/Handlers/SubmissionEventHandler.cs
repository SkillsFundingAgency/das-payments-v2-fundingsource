...
namespace SFA.DAS.Payments.FundingSource.LevyFundedProxyService.Handlers
{
    public abstract class SubmissionEventHandler<T> : IHandleMessages<T> where T : SubmissionJobFinishedEvent
    {
            ...
            ILevyMessageRoutingService levyMessageRoutingService)
        {
            ...
            this.levyMessageRoutingService = levyMessageRoutingService ??
        ...
        public async Task Handle(T message, IMessageHandlerContext context)
        {
            ...
            logger.LogInfo(
                $"Successfully processed {messageType} for Job: {message.JobId}, UKPRN: {message.Ukprn}. Skipped submission removing as no account ID found.");
        }
        ...
        private async Task InvokeSubmissionAction(long accountId, T message)
        {
            ...
            logger.LogInfo(
                $"Successfully processed {typeof(T).Name} event for Actor Id {actorId}, Job: {message.JobId}, UKPRN: {message.Ukprn}");
        }
