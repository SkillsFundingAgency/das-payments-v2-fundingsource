...
namespace SFA.DAS.Payments.FundingSource.LevyFundedProxyService.UnitTests.Handlers
{
    ...
    public class SubmissionEventHandlerTests
    {
        ...
        public void SetUp()
        {
            ...
            _eventHandler = new TestSubmissionEventHandler(_proxyFactory.Object, _levyFundingSourceRepository.Object,
                Mock.Of<IPaymentLogger>(), executionContext, _levyMessageRoutingService.Object);
        ...
        {
            var expectedAccounts = new List<Tuple<long, long?>>
                { new Tuple<long, long?>(435345, 4354353), new Tuple<long, long?>(45684532, 32497234) };
            _levyFundingSourceRepository
                .Setup(x => x.GetEmployerAccountsByUkprn(_event.Ukprn, It.IsAny<CancellationToken>()))
                .ReturnsAsync(expectedAccounts);

            _levyMessageRoutingService
                .Setup(x => x.GetDestinationAccountId(expectedAccounts[0].Item1, expectedAccounts[0].Item2))
                .Returns(expectedAccounts[0].Item2.Value);
            _levyMessageRoutingService
                .Setup(x => x.GetDestinationAccountId(expectedAccounts[1].Item1, expectedAccounts[1].Item2))
                .Returns(expectedAccounts[1].Item2.Value);

            var sender1LevyFundedService = new Mock<ILevyFundedService>();
            var sender2LevyFundedService = new Mock<ILevyFundedService>();

            _proxyFactory.Setup(x =>
                    x.CreateActorProxy<ILevyFundedService>(
                        It.Is<Uri>(y => y.ToString() == LevyFundedServiceConstants.ServiceUri),
                        It.Is<ActorId>(y => y.ToString() == expectedAccounts[0].Item2.Value.ToString()), null))
                .Returns(sender1LevyFundedService.Object);
            _proxyFactory.Setup(x =>
                    x.CreateActorProxy<ILevyFundedService>(
                        It.Is<Uri>(y => y.ToString() == LevyFundedServiceConstants.ServiceUri),
                        It.Is<ActorId>(y => y.ToString() == expectedAccounts[1].Item2.Value.ToString()), null))
                .Returns(sender2LevyFundedService.Object);
